      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="k in pagos" :key="k.id" :class="{ 'opacity-50 line-through': k.estado === 'anulado' }">
            ...
            <td class="text-right whitespace-nowrap"><a :href="`/proveedores/pagos/${k.id}/imprimir`" target="_blank" class="btn-ghost !px-2 text-xs">Imprimir</a><button v-if="k.estado !== 'anulado'" @click="anularPago(k)" class="btn-ghost !px-2 text-xs text-carmin">Anular</button></td>
          </tr>
