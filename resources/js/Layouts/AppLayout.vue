<template>
  ...
  <div class="min-h-screen flex bg-marca-fondo">
    ...
    <!-- Sidebar -->
    <aside :class="abierto ? 'w-64' : 'w-[72px]'" class="hidden md:flex flex-col shrink-0 transition-all duration-200 text-white" style="background:#1c1a18">
      <div class="h-16 flex items-center px-4 gap-3 border-b border-white/10">
      ...
      <p v-if="abierto" class="px-4 pt-3 text-[11px] font-semibold text-white/50 truncate">{{ empresa?.nombre }}</p>

      <nav class="flex-1 overflow-y-auto py-3">
        <template v-for="grupo in nav" :key="grupo.label">
          <p v-if="abierto && grupo.label" class="px-4 pt-4 pb-1 text-[10px] font-bold uppercase tracking-widest text-white/40">{{ grupo.label }}</p>
          <Link v-for="item in grupo.items" :key="item.key" :href="item.ruta"
                ...
                :class="activo(item) ? 'bg-white/10 text-white font-semibold' : 'text-white/70 hover:bg-white/5 hover:text-white'"
                :title="item.label">
            ...
            <span v-if="abierto && !item.disponible" class="ml-auto text-[9px] uppercase tracking-wide px-1.5 py-0.5 rounded bg-white/10 text-white/60">pronto</span>
          </Link>
        </template>
      </nav>
      ...
      <div class="p-3 border-t border-white/10">
        <div class="flex items-center gap-3 px-2">
          ...
          <div v-if="abierto" class="min-w-0 flex-1">
            ...
            <p class="text-[11px] text-white/60 truncate">{{ user?.rol }}</p>
          </div>
    ...
    <div v-if="movil" class="fixed inset-0 z-40 md:hidden">
      ...
      <div class="absolute left-0 top-0 bottom-0 w-72 text-white p-4 overflow-y-auto" style="background:#1c1a18">
        <div class="flex items-center justify-between mb-4">
          <Logo negativo clase="h-7" />
          <button @click="movil = false"><Icono nombre="x" /></button>
        </div>
        <template v-for="grupo in nav" :key="grupo.label">
          <p v-if="grupo.label" class="pt-3 pb-1 text-[10px] font-bold uppercase tracking-widest text-white/40">{{ grupo.label }}</p>
