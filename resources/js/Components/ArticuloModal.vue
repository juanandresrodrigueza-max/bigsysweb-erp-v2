    ...
    <div class="grid sm:grid-cols-3 gap-4">
      ...
      <div class="sm:col-span-3 grid grid-cols-2 sm:grid-cols-4 gap-3 p-3 rounded-xl bg-marca-fondo">
        ...
        <template v-if="form.usar_margenes">
          <div v-for="n in [1,2,3,4,5]" :key="'m'+n"><label class="label">Margen lista {{ n }} %</label><input v-model.number="form.margenes[n]" type="number" step="any" class="input !py-1.5 text-sm" @input="derivarPrecios" :placeholder="n > 1 ? 'vacío = lista 1' : ''" /></div>
          <div class="col-span-2 sm:col-span-4 grid grid-cols-5 gap-2 text-xs">
            <div v-for="n in [1,2,3,4,5]" :key="'p'+n" class="rounded-lg bg-white p-2"><p class="text-marca-muted">Lista {{ n }}</p><p class="font-bold tabular-nums">{{ moneda(n === 1 ? form.price : (form.prices[n] || form.price)) }}</p></div>
          </div>
        </template>
        <template v-else>
        ...
        <div class="col-span-2 sm:col-span-4 grid grid-cols-2 gap-2 pt-2 border-t border-marca-borde/60">
          <div><label class="label">Descuento por cantidad: desde</label><input v-model.number="form.desc_cant_min" type="number" step="any" min="0" class="input !py-1.5 text-sm" placeholder="Ej: 10 unidades" /></div>
          <div><label class="label">se descuenta %</label><input v-model.number="form.desc_cant_pct" type="number" step="any" min="0" max="100" class="input !py-1.5 text-sm" placeholder="Ej: 10" /></div>
        </div>
...
const vacio = () => ({ id: null, name: '', sku: '', tipo: 'producto', rubro_id: null, unit: 'un', barcode: '', marca: '', proveedor_id: null, cost: 0, price: 0, iva: 21, prices: { 2: '', 3: '', 4: '', 5: '' }, stock_min: 0, stock_inicial: '', deposito_id: null, controla_stock: true, description: '', active: true,
  precio_compra: 0, descuento_proveedor: 0, moneda: 'ARS', usar_margenes: false, margenes: { 1: '', 2: '', 3: '', 4: '', 5: '' }, desc_cant_min: 0, desc_cant_pct: 0, perecedero: false, seriado: false, en_tienda: true })
...
function derivarPrecios() {
  ...
  for (const n of [2, 3, 4, 5]) { const m = form.margenes[n]; form.prices[n] = m !== '' && m !== null ? r2(form.cost * (1 + Number(m) / 100)) : '' }
}
function guardar() {
  derivarPrecios()
