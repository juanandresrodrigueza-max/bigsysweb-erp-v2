...
class Catalogo
{
    ...
    public static function producto(Product $p, string $forma): array
    {
        ...
        return $base + match ($forma) {
            'venta' => ['iva' => (float) $p->iva, 'rubro_id' => $p->rubro_id, 'perecedero' => (bool) $p->perecedero, 'seriado' => (bool) $p->seriado, 'precios' => [1 => (float) $p->price, 2 => $p->precioLista(2), 3 => $p->precioLista(3), 4 => $p->precioLista(4), 5 => $p->precioLista(5), 6 => $p->precioLista(6)]],
