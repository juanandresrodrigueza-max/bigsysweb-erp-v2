...
class ProveedoresController extends Controller
{
    ...
    public function show(int $id)
    {
        ...
        $pendientes = Comprobante::where('contact_id', $c->id)->pendientesPago()->orderBy('fecha_vto')->get()->map(fn($x) => ['id' => $x->id, 'nombre' => $x->nombreTipo(), 'numero' => $x->numeroFormateado(), 'fecha' => $x->fecha->format('d/m/Y'), 'fecha_vto' => $x->fecha_vto?->format('d/m/Y'), 'total' => (float) $x->total, 'saldo' => (float) $x->saldo, 'moneda' => $x->moneda ?? 'ARS', 'cotizacion' => (float) $x->cotizacion, 'saldo_usd' => ($x->moneda ?? 'ARS') !== 'ARS' && (float) $x->cotizacion > 0 ? round((float) $x->saldo / (float) $x->cotizacion, 2) : null, 'vencido' => $x->fecha_vto && $x->fecha_vto->lt(today())]);
        ...
        return Inertia::render('Proveedores/Ver', [
            ...
            'cuentas' => CuentaFondos::where('activa', true)->orderBy('tipo')->orderBy('nombre')->get(['id', 'tipo', 'nombre', 'saldo', 'moneda']), 'cotizacionUsd' => \App\Models\Cotizacion::valor(\Illuminate\Support\Facades\Auth::user()->business_id),
