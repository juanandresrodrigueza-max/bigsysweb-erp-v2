...
class ContadorController extends Controller
{
    public function index(Request $request)
    {
        ...
        return Inertia::render('Contable/Contador', [
            ...
            'contadores' => User::where('business_id', $b->id)->where('role_id', $rolContador?->id)->get()->map(fn($u) => ['id' => $u->id, 'name' => $u->name, 'email' => $u->email, 'status' => $u->status, 'ultimo' => $u->last_login_at?->diffForHumans() ?? 'nunca entró']),
            'resumen' => [
    ...
    public function invitar(Request $request)
    {
        ...
        $rol = Role::where('slug', 'contador')->firstOrFail();
