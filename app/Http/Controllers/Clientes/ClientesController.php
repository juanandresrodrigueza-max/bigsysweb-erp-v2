...
class ClientesController extends Controller
{
    public function index(Request $request)
    {
        ...
        return Inertia::render('Clientes/Index', [
            ...
            'condicionesIva' => Contact::CONDICIONES_IVA, 'paises' => config('arca_paises.paises'), 'cuitPais' => config('arca_paises.cuit_pais'),
    ...
    public function show(int $id, Request $request)
    {
        ...
        return Inertia::render('Clientes/Ver', [
            'cliente' => $c->only('id', 'name', 'cuit', 'condicion_iva', 'email', 'phone', 'mobile', 'address', 'city', 'province', 'postal_code', 'credit_limit', 'lista_precios', 'dias_pago', 'descuento', 'percepcion_iibb', 'percepcion_iva', 'percepcion_ganancias', 'pais_codigo', 'cuit_pais', 'id_impositivo', 'balance', 'is_active', 'notes', 'tipo_cliente_id', 'interes_mora', 'vendedor_id', 'recordar_precio', 'cobrador_id') + ['tipo' => $c->tipoCliente?->nombre, 'deuda_vencida' => $c->deudaVencida(), 'vendedor' => $c->vendedor?->nombre, 'interes_calculado' => $this->interesMora($c, $pendientes), 'portal_url' => \App\Http\Controllers\PortalController::url($c), 'puntos' => (float) $c->puntos, 'puntos_pesos' => app(\App\Services\Ventas\FidelizacionService::class)->valorEnPesos($request->user()->business, (float) $c->puntos), 'fidelizacion_activa' => (bool) app(\App\Services\Ventas\FidelizacionService::class)->config($request->user()->business)['activo']],
            'cc' => $cc, 'pendientes' => $pendientes, 'antiguedad' => $antiguedad,
            'rubros' => \App\Models\Rubro::with('parent')->orderBy('nombre')->get()->map(fn($r) => ['id' => $r->id, 'nombre' => $r->nombreCompleto()])->sortBy('nombre')->values(),
            ...
            'medios' => Cobro::MEDIOS, 'tipos' => TipoCliente::orderBy('nombre')->get(['id', 'nombre']), 'condicionesIva' => Contact::CONDICIONES_IVA, 'paises' => config('arca_paises.paises'), 'cuitPais' => config('arca_paises.cuit_pais'),
            'cuentas' => \App\Models\CuentaFondos::where('activa', true)->orderBy('tipo')->orderBy('nombre')->get(['id', 'tipo', 'nombre', 'saldo', 'moneda']), 'cotizacionUsd' => \App\Models\Cotizacion::valor($request->user()->business_id),
            'vendedores' => \App\Models\Vendedor::where('activo', true)->orderBy('nombre')->get(['id', 'nombre']),
        ]);
    }
    ...
    public function guardar(Request $request, ?int $id = null)
    {
        ...
        $data = $request->validate([
            ...
            'credit_limit' => 'nullable|numeric|min:0', 'lista_precios' => 'nullable|integer|min:1|max:6', 'dias_pago' => 'nullable|integer|min:0|max:365', 'interes_mora' => 'nullable|numeric|min:0|max:100', 'vendedor_id' => 'nullable|exists:vendedores,id', 'cobrador_id' => 'nullable|exists:vendedores,id',
            'descuento' => 'nullable|numeric|min:0|max:100', 'percepcion_iibb' => 'boolean', 'percepcion_iva' => 'boolean', 'percepcion_ganancias' => 'boolean', 'pais_codigo' => 'nullable|string|max:4', 'cuit_pais' => 'nullable|string|max:15', 'id_impositivo' => 'nullable|string|max:40', 'is_active' => 'boolean', 'notes' => 'nullable|string|max:2000', 'recordar_precio' => 'boolean',
        ]);
