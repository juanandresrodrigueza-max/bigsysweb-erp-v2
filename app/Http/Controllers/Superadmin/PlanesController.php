...
class PlanesController extends Controller
{
    public function index()
    {
        return Inertia::render('Superadmin/Planes', [
            'planes' => Plan::withCount(['subscriptions as activas' => fn($q) => $q->whereIn('status', ['trial', 'active', 'grace'])])->orderBy('price_monthly')->get()->map(fn($p) => [
                ...
                'max_users' => $p->max_users, 'max_locations' => $p->max_locations, 'max_products' => $p->max_products, 'max_facturas_mes' => (int) $p->max_facturas_mes, 'features' => (array) $p->features, 'is_active' => $p->is_active, 'is_free' => $p->is_free, 'activas' => $p->activas,
            ]),
    ...
    public function guardar(Request $request, ?int $id = null)
    {
        $d = $request->validate([
            ...
            'max_users' => 'required|integer|min:-1', 'max_locations' => 'required|integer|min:-1', 'max_products' => 'required|integer|min:-1', 'max_facturas_mes' => 'nullable|integer|min:-1',
