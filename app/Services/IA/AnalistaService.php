...
class AnalistaService
{
    public function hallazgos(Business $b): array
    {
        ...
        $vencido = Comprobante::withoutGlobalScopes()->where('business_id', $id)->where('direccion', 'venta')->where('estado', 'emitido')->where('saldo', '>', 0.005)->where('fecha_vto', '<', hoy)->selectRaw('contact_id, SUM(saldo) s, MIN(fecha_vto) v')->groupBy('contact_id')->orderByDesc('s')->get();
        ...
        $horas = Comprobante::withoutGlobalScopes()->where('business_id', $id)->where('direccion', 'venta')->where('estado', 'emitido')->whereNotNull('emitido_en')->where('fecha', '>=', $hoy->copy()->subDays(30))->selectRaw("" . \App\Support\Sql::hora('emitido_en') . " h, COUNT(*) n")->groupBy('h')->orderByDesc('n')->get();
